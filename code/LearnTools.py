...
def calc_gradient_penalty_MP(netD, real_data, fake_data, batch_size, l, device, gp_lambda, nc, scaler):
    """
    calculate gradient penalty for a batch of real and fake data
    :param netD: Discriminator network
    :param real_data:
    ...
    alpha = torch.rand(batch_size, 1, device=device)
    num_images = real_data.size()[0]
    alpha = alpha.expand(batch_size, int(real_data.numel() / batch_size)).contiguous()
    alpha = alpha.view(num_images, nc, l, l)
    ...
    # pass interpolates through netD
    disc_interpolates = netD(interpolates)
    gradients = autograd.grad(outputs=scaler.scale(disc_interpolates), inputs=interpolates, grad_outputs=torch.ones(disc_interpolates.size(), device=device),
                              create_graph=True, only_inputs=True)[0]
...
#        device1 = torch.device("cuda:0")                                      ########################################
#        generated_im = generated_im.to(device1)                               ########################################
        ...
        if self.squash:  # all phases of material are same in low-res
            ...
            low_res_phases = torch.sum(low_res_phases, dim=1).unsqueeze(dim=1)
        # if it is super-sampling, return nearest-neighbour interpolation:
        if self.super_sampling:
            return interpolate(low_res_phases, scale_factor=1 / self.scale_factor, mode='nearest')
...
class DownSample_2(nn.Module):
    ...
    def __init__(self, squash, n_dims, low_res_idx, scale_factor,
        ...
        self.n_dims = n_dims
        # Here we want to compare the pore as well:
        self.low_res_idx = torch.cat((torch.zeros(1).to(low_res_idx), low_res_idx))
...
#        device1 = torch.device("cuda:0")                                     ########################################
#        low_res = low_res.to(device1)                                        ########################################
        # mse_loss = torch.nn.MSELoss()(low_res[:, 0], down_sampled_im)
        mse_loss = torch.nn.MSELoss()(torch.index_select(low_res, 1, self.low_res_idx), down_sampled_im)
        return mse_loss * self.low_res_len / 2  # to standardize the loss.
