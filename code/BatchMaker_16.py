...
crop_size = 8  # crop pixels in each dimension when choosing train slices
HIGH_L_3D = [32, 64, 128, 256, 512]  # length of high resolution 2d - to make it for different stages. 0 is dummy, to use the stage variable
...
class BatchMaker:
    ...
    def __init__(self, stage, device, to_low_idx=None, path=NMC_PATH, sf=4, dims=3,
        ...
        self.im = imread(path)
        self.high_l = HIGH_L_3D[stage]
        if stack and not low_res and stage == 4:  # it is the high-res training data            
            self.hr_metrics = ImageTools.vf_sa_metrics(self.im[:10])
            # self.high_l -= crop_size*2 # here i decided make the whole images
        if rot_and_mir:
        ...
        if low_res: # this one is for the 3D LR sampling. Here it should be constant all the time, for all of the stages -> fix it with 4
            self.high_l = int(HIGH_L_3D[4]/self.scale_factor)
        if self.dims == 2:
            self.high_l = self.high_l*2
        if self.down_sample: # this one is always False in my option therefore no point to worry about it
    ...
    def random_batch2d(self, batch_size):
        ...
        along the dimension chosen (0->x,1->y,2->z) in the 3d tif image.
        """
        res = np.zeros((batch_size, len(self.phases), self.high_l, self.high_l), dtype=self.im.dtype)
        for i in range(batch_size):
            res[i, :, :, :] = self.generate_a_random_image2d()

        res = torch.FloatTensor(res).to(device=self.device)
        if self.stage == 4:
            zeros_to_append = torch.zeros(res.size()[0], 1, res.size()[2], res.size()[3]).to(device=self.device)  ###
            res = torch.cat((res, zeros_to_append), dim=1)                   ### because here we have the final channel for the mixed ###
        return res
    ...
    def generate_a_random_image2d(self):
        ...
        if self.stack:  # has to be, since this function is only called for
            ...
            s_ind = np.random.randint(np.array(self.im.shape[2:]) - self.high_l)
            e_ind = s_ind + self.high_l
            slice_chosen = np.random.randint(self.im.shape[1])
            return self.im[:, slice_chosen, s_ind[0]:e_ind[0], s_ind[1]:e_ind[1]]
        else:
            raise ValueError("2D images should be stacked as n_stack X height X width")
